...
let g:ant_complete_item_len = 30
...
function! antcore#complete(findstart, base)
  if a:findstart
    let line = line('.')
    let cur = col('.') - 1
    ...
    if s:complete_mode == s:MODE_NAMESPACE
      call antcore#ns_completion(a:base, res)
    ...
    elseif s:complete_mode == s:MODE_CLASS
      ...
      call antcore#enum_completion(a:base, res)
    ...
    elseif s:complete_mode == s:MODE_STATIC
      ...
      if len(s:parts) >= 1 
        ...
        for ns in antcore#getNamespaces()
          if ns =~ '^' . variable
            let compitem = ns[start : ]
            call add(res, antcore#ns_to_compitem(compitem))
          endif
          if ns == type
            let match_ns = 1
          endif
        ...
        elseif antcore#isEnumExist(s:parts[0])
...
function! antcore#ns_completion(base, res)
  for ns in s:namespace
    if ns =~ '^' . a:base
      call add(a:res, antcore#ns_to_compitem(ns))
    endif
  endfor
endfunction
...
function! antcore#class_completion(base, res)
...
function! s:enum_member_completion(tag, base, res, type)
  if !antcore#isEnumExist(a:tag)
    return
  endif

  let item = antcore#getEnum(a:tag)
  for member in item.members
   if member.name =~ '^' . a:base
      let newitem = antcore#member_to_compitem(item.name, member)
...
function! antcore#enum_completion(base, res)
...
function! s:class_member_completion(base, res, type)
  ...
  for part in parts
    ...
    if !antcore#isClassExist(class)
      if !antcore#isEnumExist(class)
"       let item = antcore#getTag(class)
"       if empty(item)
"         if exists('item')
"           unlet item
"         endif
...
"       endif
        ...
        let item = antcore#getEnum(class)
      endif
    else
      let item = antcore#getClass(class)
    endif
    ...
    while 1
      ...
      if has_key(item, 'extend')
        if antcore#isClassExist(item.extend)
          let item = antcore#getClass(item.extend)
        else
"         let item = antcore#getTag(item.extend)
"         if empty(item)
"           unlet item
            break
"         endif
  ...
  if exists('item')
    ...
    elseif a:type == 1
      ...
      if !has_key(s:primitive_dict, item.name)
        let newitem = antcore#member_to_compitem('new ' . item.name . '(', {})
        let newitem.menu = 'create new instance'
        call add(a:res, newitem)
        let newitem = antcore#member_to_compitem('(' . item.name . ')', {})
        let newitem.menu = 'for cast'
        call add(a:res, newitem)
      endif
      if empty(a:res)
        call antcore#class_completion(a:base, a:res)
      endif
    else
...
function! s:find_type(start_line, var)
  ...
  if antcore#isClassExist(result.class)
...
function! s:abbr(str)
  if len(a:str) > g:ant_complete_item_len
    return a:str[0 : g:ant_complete_item_len] . '...'
  endif
  return a:str
endfunction

function! antcore#member_to_compitem(class, member)
  if empty(a:member)
    return {
      \ 'word' : a:class,
      \ 'abbr' : s:abbr(a:class),
...
function! antcore#ns_to_compitem(ns)
  return {
    \ 'word' : a:ns,
    \ 'menu' : 'namespace',
    \ 'kind' : 't',
...
let s:class = {}
function! antcore#class(name, extend, members)
  let s:class[ a:name ] = s:def_class(a:name, a:extend, a:members)
  if exists('s:parent') && index(s:parent.members, a:name) == -1
    call add(s:parent.members, antcore#prop(a:name, a:name))
  endif
endfunction

let s:namespace = []
function! antcore#namespace(ns)
  try
    unret s:parent
  catch /.*/
  endtry
  ...
  for part in parts
    ...
    "  call add(s:parent.members, antcore#prop(part, part))
    "endif

    if !antcore#isClassExist(part)
      call s:namespace_item(part, '', [])
    endif

    " last namespace (for antcore#class)
    let s:parent = antcore#getClass(part)
  endfor
endfunction

let s:keyword = []
function! antcore#keyword(name, detail)
  call add(s:keyword, 
    \ {
    \ 'type'      : s:TYPE_KEYWORD,
    \ 'name'      : a:name,
...
function! antcore#method(name, detail, class)
  return {
    \ 'type'   : s:TYPE_METHOD,
    \ 'kind'   : 'f', 
    \ 'name'   : a:name,
...
function! antcore#prop(name, class)
  return {
    \ 'type'   : s:TYPE_PROP,
    \ 'kind'   : 'm', 
    \ 'name'   : a:name,
...
function! antcore#attr(name, class)
  return {
    \ 'type'   : s:TYPE_ATTR,
    \ 'kind'   : 'm', 
    \ 'name'   : a:name,
...
function! antcore#const(name, value)
  return {
    \ 'type'   : s:TYPE_CONST,
    \ 'kind'   : 'm', 
    \ 'name'   : a:name,
...
function! antcore#field(name, class)
  return {
    \ 'type'   : s:TYPE_FIELD,
    \ 'kind'   : 'v', 
    \ 'name'   : a:name,
...
function! antcore#event(name, class)
  return {
    \ 'type'   : s:TYPE_EVENT,
    \ 'kind'   : 'f',
    \ 'name'   : a:name,
...
let s:enum = {}
function! antcore#enum(name, members)
  let s:enum[ a:name ] = {
    \ 'type'   : s:TYPE_ENUM,
    \ 'name'   : a:name,
    \ 'kind'   : 't',
...
let s:binding = {}
function! antcore#bind(name, members)
...
function! antcore#isMethod(member)
...
function! antcore#isConst(member)
...
function! antcore#isAttr(member)
...
function! antcore#isEvent(member)
...
function! antcore#isClassExist(name)
  return has_key(s:class, a:name)
endfunction

function! antcore#getClass(name)
  return get(s:class, a:name)
endfunction

function! antcore#getSuperClassList(name, list)
  if !antcore#isClassExist(a:name)
    return
  endif

  let item = antcore#getClass(a:name)
  if item.extend != '' && item.extend != '-'
    call add(a:list, item.extend)
    call antcore#getSuperClassList(item.extend, a:list)
  endif
endfunction

" その都度tagsからロードしていると遅いので、
" タグ → s:class にロードして使うようにする
" function! antcore#getTag(name)
...
"       let ttype = name
"     endif
"     call add(class.members, antcore#prop(name, ttype))
"   endfor
"   return class
" endfunction

function! antcore#isBindingExist(name)
  return has_key(s:binding, a:name)
endfunction

function! antcore#getBinding(name)
  return get(s:binding, a:name)
endfunction

function! antcore#isEnumExist(name)
  return has_key(s:enum, a:name)
endfunction

function! antcore#getEnum(name)
  return get(s:enum, a:name)
endfunction

function! antcore#getNamespaces()
  return s:namespace
endfunction
...
function! antcore#showRef()
  if !exists('g:antcore#statusline')
    let g:antcore#statusline = &statusline
  endif
  ...
  \ 'line'  : line('.'),
  \ }
  call antcore#nextRef()

  augroup antcore
    au!
    au InsertLeave  <buffer> call antcore#clearRef()
    au CursorMovedI <buffer> call antcore#checkLineForRef()
  augroup END
  return ""
endfunction
...
function! antcore#nextRef()
  return s:prevNextRef(1)
endfunction
function! antcore#prevRef()
  return s:prevNextRef(-1)
endfunction
...
function! antcore#checkLineForRef()
  if exists("b:ref")
    if b:ref.line != line('.')
      call antcore#clearRef()
    endif
  endif
endfunction

function! antcore#reshowRef()
  if exists("b:ref")
    if b:ref.line == line('.')
...
function! antcore#clearRef()
  let &l:statusline = g:antcore#statusline
  augroup antcore
    au!
    au InsertEnter  <buffer> call antcore#reshowRef()
  augroup END
endfunction

function! antcore#balloon()
      ...
      elseif has_key(titem, "class") && ((titem.kind == "f" || titem.kind == "m" || titem.kind == "p"))
        ...
        if index(defs[cname].member_names, mname) == -1
          ...
          if has_key(member, 'signature')
            ...
            let item = antcore#method(mname . '(', signature[1:], ttype)
            let mname = mname . signature
          else
            let item = antcore#prop(mname, ttype)
          endif
...
" load autoload/ant/.vim
function! antcore#load(sub)
  let rtp = filter(split(&runtimepath, ','), 'v:val =~ a:sub')
  let files = split(globpath(join(rtp, ','), 'autoload/ant/*.vim'), '\n')
  for file in files
    if file
      continue
    endif
  ...
  call s:msg('loadded')
  call remove(g:antcore#delay_dirs, a:sub)
endfunction
...
if !exists('s:dictionary_loaded')
  ...
  " delay directories
  if !exists('g:antcore#delay_dirs')
    let g:antcore#delay_dirs = []
  end
  let rtp = split(&runtimepath, ',')
  for dir in g:antcore#delay_dirs
